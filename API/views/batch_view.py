...
class BatchInfoView(APIView):
    def get(self, request):
        ...
        # 모델 리소스의 DELETE_FLAG가 True인 경우 409 에러 반환
        if get_t.DELETE_FLAG:
            return Response(error_code.CONFLICT_4009(mode='BATCH', error_msg='deleted'),
                            status=status.HTTP_409_CONFLICT)
        # 모델이 학습 종료되지 않은 경우도 409 에러 반환
        if get_t.PROGRESS_STATE != 'success':
            return Response(error_code.CONFLICT_4009(mode='BATCH', error_msg=get_t.PROGRESS_STATE),
                            status=status.HTTP_409_CONFLICT)

        get_o = get_object_or_404(OriginalData, pk=get_t.ORIGINAL_DATA_SEQUENCE_FK1)
        get_p = get_object_or_404(PreprocessedData, pk=get_t.PREPROCESSED_DATA_SEQUENCE_FK2)
...
class BatchServiceView(APIView):
    def post(self, request):
        ...
        batch_service = BatchService.objects.create(
            ...
            PREPROCESSED_DATA_SANDBOX_SEQUENCE_FK2=request_data['PREPROCESSED_DATA_SEQUENCE_FK2'],
            ORIGINAL_DATA_SANDBOX_SEQUENCE_FK3=request_data['ORIGINAL_DATA_SEQUENCE_FK3'])
...
class BatchServiceDetailView(APIView):
    ...
    def patch(self, request, pk):
        batch_service = BatchServiceSerializer(
            get_object_or_404(BatchService, pk=pk)).data
        user_request = request.data
        if 'test_data_path' not in user_request.keys():
            return Response(error_code.MANDATORY_PARAMETER_MISSING_4101('test_data_path'),
                            status=status.HTTP_400_BAD_REQUEST)
        else:
            test_data_path = user_request['test_data_path']  # 테스트 데이터의 전체 경로

            logger.info('요청한 배치 서비스 ID [{}]의 요청 정보를 확인합니다'.format(pk))
            ...
            # 예측 결과 출력
            logger.info('요청한 배치 서비스 ID [{}]의 테스트를 시작합니다'.format(pk))
