...
class TrainModelView(APIView):
    def post(self, request):
        ...
        logger.info('요청 ID [{}]의 모델 학습을 시작합니다'.format(get_pk_new))
    ...
    def patch(self, request, pk):
        ...
        # 수정하는 경우 1 = 모델 학습 중지
            ...
            if serializer.data['PROGRESS_STATE'] == 'success':
                ...
                logger.info('요청 ID [{}]의 모델 학습을 중지합니다'.format(pk))
                ...
                if serializer.is_valid():
                    serializer.save()
                    return Response({'request_id': pk, 'progress_state': 'standby'},
        ...
        # 데이터 전달 및 데이터 적용 테스트
            ...
            if serializer.data['PROGRESS_STATE'] != 'success':
                return Response(
                    error_code.CONFLICT_4009(mode='TEST', error_msg=serializer.data['PROGRESS_STATE']),
                    status=status.HTTP_409_CONFLICT)

            base_directory = settings.NIFI_RESULT_DIRECTORY # /home/centos/NIFI_RESULT

            request_data_path = os.path.join(base_directory, user_request['test_data_path'])
            if not os.path.isfile(request_data_path):
                return Response(error_code.FILE_NOT_FOUND_4004(path_info=request_data_path),
                                status=status.HTTP_404_NOT_FOUND)

            # 예측값과 스코어를 같이 출력함
            logger.info('요청한 모델 ID [{}]의 테스트를 시작합니다'.format(pk))
            train_y = ast.literal_eval(serializer.data['COMMAND'])['train_parameters']['y']
