...
class TestPreprocessor(PreprocessorBase):
    ...
    For processing test result of Preprocessor from original Data
        ...
        Attributes:
        -----------
            func_query (dict) : preprocessFunction Query
            original_data_pk (int) : user requested ID of original Data
            test_total_result (list) : final return value of user's
                                       requested about Preprocessor Testing
            test_data (pandas.DataFrame) : user requested Data
                                           for Preprocessor Testing
    """
    ...
    def __init__(self, file_name, pk):
        ...
        self.test_data = super().load_data(file_name)
    ...
    def _mandatory_key_exists_original_patch(element):
        ...
        Check whether mandatory keys are existed
            ...
            Parameters:
            -----------
                 element (dict) : raw request information
                                  from user's request body

            Returns:
            --------
                 True (bool) : True, if all mandatory keys satisfied
                 or
                 _key or mand_key_level_one (str) : omitted key
        """
    ...
    def _convert_to_return_shape(data_shape, after_):
        """
        Convert returning shape of data('after_') by data's original shape and type

            Parameters:
            -----------
                 data_shape (tuple) : data shape from numpy.ndarray.shape
                 after_ (numpy.ndarray) : array shape data after specific
                                          transformers were applied

            Returns:
            --------
                 changed_field (list) : list of flatten array which element
        ...
        if isinstance(after_[1], np.ndarray) and data_shape[1] == 1:
            ...
            changed_field = [round(float(i), 4) if str(i)[::-1].find('.') > 4 else i for i in changed_field]
    ...
    def _test_transformer(self, field, field_name, transformer):
        """
        Perform fit_transform with specific field with requested transformer

            Parameters:
            -----------
                 field (pandas.Series) : specific data values selected by field name
                 field_name (str) : field name of field
                 transformer (object) : preprocessor from scikit-learn library

            Returns:
            --------
                 show_changed_field (dict) : converted value of changed_field
                                             by '_convert_to_return_shape' function
                 or
                 return_error (dict) : specific error info to induce custom error
        """
        ...
        try:
            ...
            show_changed_field = self._convert_to_return_shape(data_shape=changed_field.shape,
                                                               after_=changed_field)

            show_changed_field = dict(zip(range(0, len(show_changed_field)), show_changed_field))
            ...
            return_error = dict(error_name='PreprocessorTestError', error_detail=error_detail)
            return return_error
    ...
    def _test_result_drop_columns(self, func_name, name_field):
        ...
        for single_field_name in field_name_list:
            self.test_data = drop_columns(data=self.test_data, columns=single_field_name)
            fake_drop_columns = [' '] * 5
            fake_drop_columns = dict(zip(range(0, len(fake_drop_columns)), fake_drop_columns))
            single_result = {'field_name': single_field_name,
    ...
    def _test_result(self, func_name, name_field, request_detail):
        """
        According to 'request_detail' prepare and request '_test_transformer'

            Parameters:
            -----------
                 func_name (str) : transformer name
                 name_field (str) : field name of field
                 request_detail (dict) : request information from user's request body

            Returns:
            --------
                 (append new value to self.test_total_result)
                 or
                 _error_return_dict() (dict) : specific error info to induce custom error
        """
        transformer = super()._get_transformer(params=self.func_query)
        ...
        for single_field_name in field_name_list:
            ...
            if single_field_name not in list(self.test_data.columns):
                return _error_return_dict('4104', [single_field_name, func_name, 'Field Name is Not Exist'])
            try:
            ...
            after_changed = self._test_transformer(field=field_column,
    ...
    def check_request_body_original_patch(self, request_info):
        ...
        Checking whether 'request_info' is valid
            ...
            Parameters:
            -----------
                 request_info (dict) : request information from user's request body
        ...
        for request_info_dict in request_info_list:
            ...
            if not int(pfunc_id) in list(PreprocessFunction.objects.all().values_list(
    ...
    def test_result(self, request_info):
        """
        According to 'request_info' invoking required function and returning result
            ...
            Returns:
            --------
                 self.test_total_result (list) : final return value of user's
                                                 requested about Preprocessor Testing
                 or
                 _error_return_dict() (dict) : specific error info to induce custom error
        """
        ...
        for get_request_dict in user_request_dict:
            ...
            self.func_query = PreprocessFunctionSerializer(PreprocessFunction.objects.get(
                pk=get_request_dict['preprocess_functions_sequence_pk'])).data
            func_name = self.func_query['PREPROCESS_FUNCTIONS_NAME']

            if func_name == 'DropColumns':  # self.test_total_result is changed
                self._test_result_drop_columns(func_name=func_name,
                                               name_field=field_name)
            else:  # return Error or self.test_total_result is changed
                is_error = self._test_result(func_name=func_name,
                                             name_field=field_name,
                                             request_detail=get_request_dict)

                if isinstance(is_error, dict):
                    return _error_return_dict(is_error['error_type'],
                                              is_error['error_msg'])
